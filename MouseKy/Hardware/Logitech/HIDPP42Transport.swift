...
final class HIDPP42Transport: @unchecked Sendable {
    private static let logger = Logger(subsystem: "io.github.stvn-pxl.MouseKy", category: "HIDPP")
    ...
    private func submitReport(_ report: Data) throws {
        ...
        Self.logger.debug("TX report=0x\(String(format: "%02X", reportID), privacy: .public) length=\(report.count, privacy: .public)")
    ...
    private func receive(reportID: UInt8, report: Data) {
        Self.logger.debug("RX report=0x\(String(format: "%02X", reportID), privacy: .public) length=\(report.count, privacy: .public)")
